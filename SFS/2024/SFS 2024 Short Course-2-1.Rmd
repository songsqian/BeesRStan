...
1. Repeat the posterior simulation to determine where/when the model is useful.
...
Section 4.2.2, Qian, DuFour, and Alameddine (2022).
...
##save(stan.fit, file="GOMstan1.RData")
```
```{R, include=F}
##load("GOMstan1.RData")
```
...
##save(abun_stan, rich_stan, file="hypoxia1.RData")
##load("hypoxia1.RData")
...
We noticed warning messages about divergent transition. It is a sign of computational difficulties in sampling the posterior distribution.

```{R GOM_m1_Neals_funnel, fig.height=3, fig.width=5, dev="tikz"}
par(mfrow=c(1,2),mar=c(3,3,1,1),
    mgp=c(1.25,0.125,0), las=1,tck=0.01)
smpl <- sample(1:4000, 400)
plot(rich_stan$mu_hyp[smpl], log(rich_stan$sigma_hyp[smpl]),
     cex=0.5, ylim=c(-3.5,log(3)), xlim=c(-0,5),
     xlab="$\\mu_{hyp}$", ylab="$\\log(\\sigma_{hyp})$")
text(1.5,-3, "Richness")
plot(abun_stan$mu_hyp[smpl], log(abun_stan$sigma_hyp[smpl]),
     cex=0.5,
...
##save(stan.fit2, file="ROMstan2.RData")
##load("ROMstan2.RData")
...
fit2keep <- sampling(stan.fit2, data = input.to.stan$data,
                     ...
                     chains=input.to.stan$n.chains)
print(fit2keep)
rich_stan2 <- rvsims(as.matrix(as.data.frame(rstan::extract(fit2keep))))

##save(rich_stan2, file="hypoxiaR2.RData")
##load("hypoxiaR2.RData")
...
##save(abun_stan2, file="hypoxiaA2.RData")
##load("hypoxiaA2.RData")
...
##save(stan.fit3, file="ROMstan3.RData")
##load("ROMstan3.RData")
...
##save(rich_stan3, file="hypoxiaR3.RData")
##load("hypoxiaR3.RData")
...
##save(abun_stan3, file="hypoxiaA3.RData")
##load("hypoxiaA3.RData")
...
```{R gom_compare, tidy=TRUE, fig.width=5, fig.height=4.75, dev="tikz"}
...
```{R gom_compare_2, tidy=TRUE, fig.width=5, fig.height=3.75, dev="tikz"}
...
## Summary
In this example, we fit the same data with one MLE and three different Bayesian models. The MLE model inadequately fits the data providing little information on the zone difference, but the summary statistics do not provide insight on where/if the model has gone wrong. Using the first Bayesian model, we see through the presence of Neal's Funnel, that the model is having trouble estimating the upper level parameters. Even when we reparameterize the model to aid convergence, Neal's funnel persists. The example demonstrates that the data we have are not guaranteed to support the complexity of a model we desire.  Compromize is necessary.  In this case, we simplified the hierarchical structure, assuming exchangeability across cores,and reparameterized the two alternative models to improve convergence efficiency and reduced estimation uncertainty. The two alternative models allow us to make inferences about the impact of hypoxia on species richness and abundance.
