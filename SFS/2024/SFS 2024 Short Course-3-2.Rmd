...
```{R euse,tidy=F, fig.width=5, fig.height=5}
...
```{R euse lmer,tidy=F, fig.width=3.25, fig.height=5}
...
```{R euse compare, tidy=F, fig.width=4.5, fig.height=4}
...
```{R euse group-level, tidy=F, width=5.75, height=4}
...
```{R euse group-level 2, tidy=F, width=5.75, height=4}
...
```{R euse group-level 3, tidy=F, width=5.75, height=4}
ag.cat2 <- ag.full>0.5
...
$$
\mathbf{\gamma} = \begin{pmatrix} a_0 & b_0 \\ a_1 & b_1\\a_3&b_3 \\ a_4 & b_4 \end{pmatrix}
$$ 
...
```{R euse Stan figures, tidy=F, height=3, width=5}
