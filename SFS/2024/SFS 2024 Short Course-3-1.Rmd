...
## Introduction
In this example we explore the benefits of aggregation. Aggregating data during the modeling process relies on an assumption of exchangeability among units. Aggregating data into units depends on our objective knowledge (what we know), subjective ignorance (what we don't know), and the objectives of the study. The basic tenet of the exchangeability assumption is that we acknowledge there are likely differences among units, but we don't know the source of these differences... yet. Assuming exchangeability among units also improves estimation performance as demonstrated by Stein's paradox and avoids the trappings of Simpson's paradox.
...
```{R everg data, tidy=TRUE, fig.width=3.25, fig.height=4, dev="tikz"}
...
par(mfrow=c(1,1), mar=c(3, 7, 1, 0.5), mgp=c(1.25,0.25,0),tck=0.01)
...
$$
which represents a common prior distribution for $\theta_j$. This hierarchical model extends the concept of Stein's paradox from the 1960s. With 12 thresholds to estimate simultaneously, Stein's paradox informs us that estimating them individually is mathematically inadmissible. By shrinking the individually estimated means towards the overall mean, we can improve the accuracy of the overall estimation. Hierarchical modeling has shown its value in applied fields since Stein's paradox, particularly in addressing environmental and ecological data analysis problems that involve variables representing different levels of spatial, temporal, and organizational aggregations. Failing to properly address data hierarchy can lead to Simpson's paradox - where patterns in the data are different between pooled and aggregate units. 
...
```{r everg stan, tidy=TRUE, message=F}
everg_stan <- "
data {
  int<lower=0> J; // number of metrics
...
##save(fit1, file="evergstan.RData")
##load("evergstan.RData")
```
...
```{R everg stan output, tidy=TRUE, fig.width=3.25, fig.height=4, dev="tikz"}
everg_fit1 <- rvsims(as.matrix(
    as.data.frame(rstan::extract(fit2keep, permuted=T))))

## shrinkage effect
everg_theta <- rvsims(as.matrix(as.data.frame(rstan::extract(fit2keep,permuted=T,pars="theta"))))
...
plot(range(y.hat-1*sigma.hat, y.hat+1*sigma.hat),
     ...
     xlab="TP Threshold", ylab=" ", axes=F)
axis(1, cex.axis=0.75)
axis(2, at=seq(1,length(y.hat)), labels=metricsTEX, las=1, cex.axis=0.75)
segments(x0=y.hat+sigma.hat, x1=y.hat-sigma.hat,
...
```{R everg env_crit, tidy=TRUE, fig.width=3.25, fig.height=3.5, dev="tikz"}
## mu versus N(mu, tau)
mu_tau <- rvnorm(1, everg_mu, everg_tau)
p1 <- hist(sims(everg_mu)[,1], freq=F, xlab="$\\mu$", main="")
p2 <- hist(sims(mu_tau)[,1], nclass=35, xlab="$\\tau$", main="")
...
## Summary
In this example we introduced several new terms: exchangeability, objective knowledge, subjective ignorance, Stein's paradox, Simpson's paradox, and Empirical Bayes. One benefit of the iterative Bayesian modeling approach we advocate, is that each of these concepts can be explicitly acknowledged and addressed through the modeling process. And we believe, particularly with large scale cross sectional data, will result in improved models tuned to the studies objectives.