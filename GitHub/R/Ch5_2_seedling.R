...
tikz(file=paste(plotDIRch5, "seedInt1and2.tex", sep="/"),
     height=3.5, width=5, standAlone=F)
par(mfrow=c(1,2), mar=c(3, 3, 1, 0.5), mgp=c(1.25,0.125,0),
    las=1,tck=0.01)
mlplot(t(alp1), xlab="", cex=0.5)
mlplot(alp1, xlab="", cex=0.5)
dev.off()
...
fit2keep4 <- sampling(fit4, data = input.to.stan4$data,
                     ...
                     control=list(adapt_delta=0.95))
print(fit2keep4)
save(fit2keep4, file="SparseCAR.RData")
load("SparseCARWin.RData")
...
sType_Pos <- sd.rv(alpha1)
sSoilC <- abs(alpha2)*sd(input.to.stan3$data$soilC)
sGap <- abs(alpha3)*sd(input.to.stan3$data$gap)
sInt2 <- sd.rv(Int2)
sInt3 <- sd.rv(Int3)
sPhi3 <- sd.rv(seedStan3phi)

vcomp3 <- c(sType_Pos, sSoilC, sGap, sInt2, sInt3, sPhi3)
names(vcomp3) <- c("Type:Position", "Soil C",
                   "Gap", "Type:Soil C", "Type:Gap", "CAR")
tikz(file=paste(plotDIRch5, "seedVC3slowCAR.tex", sep="/"),
     height=2.75, width=3.75, standAlone=F)
par(mar=c(3,3,1,0.25), mgp=c(1.25,0.125,0), las=1,tck=0.01)
...
## table:
write.table(round(rbind(
summary(seedStan2beta0),
summary(seedStan2beta2),
summary(seedStan2beta3),
summary(seedStan4beta0),
summary(seedStan4beta2),
summary(seedStan4beta3))[,-1], 3), sep="&", file=paste(plotDIRch5, "seedCARcomp.txt", sep="/"))

tikz(file=paste(plotDIRch5, "seedIntCAR.tex", sep="/"),
     height=3.5, width=5, standAlone=F)
par(mfrow=c(1,2), mar=c(3, 3, 1, 0.5), mgp=c(1.25,0.125,0),
    las=1,tck=0.01)
mlplot(t(alp1), xlab="", cex=0.5)
